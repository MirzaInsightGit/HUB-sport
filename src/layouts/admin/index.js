  ...
  const getRoutes = (routes) => {
    return routes.map((route, key) => {
      if (route.layout === '/admin') {
        const normalized = typeof route.path === 'string' && route.path.startsWith('/')
          ? route.path.slice(1)
          : route.path;
        return (
          <Route path={normalized} element={route.component} key={key} />
        );
      }
      if (route.collapse) {
      ...
      <Box>
        ...
        >
          ...
          <Box
            ...
            position="relative"
            maxHeight="100%"
            ms={{ base: '0', xl: SIDEBAR_WIDTH }}
            w={{ base: '100%', xl: `calc(100% - ${SIDEBAR_WIDTH})` }}
            maxWidth={{ base: '100%', xl: `calc(100% - ${SIDEBAR_WIDTH})` }}
